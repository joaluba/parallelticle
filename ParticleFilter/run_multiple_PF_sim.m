% This is a function run a multiple particle filters
% (as documented in the proof of concept)
% and save/plot the results
%
% ------------------ INPUT: -----------------
...
% sResult.             - matrices containing input data that
%                             each PF receives
% sResult.T_s1_EST/T_s2_EST - matrices containing evolution of the expected
%                             value of the state for voice 1 and 2
% sResult.T_s1_entr/T_s2_entr - matrices containing evolution of the 
%                                               entropy of the particles
% sResult.T_s1_var/T_s2_var - matrices containing evolution of the 
%                                               variance of the particles
% sResult.winner_EST - which particle filter won each iteration
%
% ------------------ USAGE: -----------------
% my_sResult=run_multiple_PF_sim(sConf,T_o,0,0)
% my_sResult=run_multiple_PF_sim(sConf,T_o,1,1,'runname')
% -----------------------------------------------------------------------
% Author: joanna.luberadzka@uni-oldenburg.de

function sResult=run_multiple_PF_sim(sConfPF1,sConfPF2,T_o,b_saveresult,b_plot,varargin)
...
sResult.T_s1_EST=nan(N,D_y);
sResult.T_s2_EST=nan(N,D_y);
sResult.T_s1_var=nan(N,D_y);
sResult.T_s2_var=nan(N,D_y);
sResult.T_s1_entr=nan(N,1);
sResult.T_s2_entr=nan(N,1);
sResult.winner_EST=nan(N,1);
...
for n=2:N
    ...
    % instead of the whole evolution:
    sResult.winner_EST(n)=winner;
    sResult.T_s1_entr(n,:)=-(sum(log(Particles1_temp.W).*Particles1_temp.W));
    sResult.T_s2_entr(n,:)=-(sum(log(Particles1_temp.W).*Particles1_temp.W));
    m_expval1=repmat(s1_EST,[1,sConfPF1.K]);
    m_expval2=repmat(s2_EST,[1,sConfPF2.K]);
    sResult.T_s1_var(n,:)=((Particles1_temp.H-m_expval1).^2)*Particles1_temp.W';
    sResult.T_s2_var(n,:)=((Particles2_temp.H-m_expval2).^2)*Particles2_temp.W';
